...
namespace info {
	//defines & datas
	const int N=5010,M=85010,inf=0x3f3f3f3f;
	const int MAXT=10010,MAXP=80,MAXD=1100,MAXR=38000;
	int n,m,T,P,D,R;
	int tasks[MAXR],taskt[MAXR],repath[MAXR];
	...
	void getinput() {
		...
		int res=0;
		for(int i=0; i<T; ++i){
			scanf("%d%d%d",&tasks[i],&taskt[i],&repath[i]);
			//if(tasks[i]>taskt[i]) swap(tasks[i],taskt[i]);
			for(int j=res;j<res+repath[i];j++){
				hw[j]=i;
			}
			rehw[i]=res;
			res+=repath[i];
			// PII tmp={tasks[i],taskt[i]};
...
namespace core {
	...
	int way[MAXR];				//result_way record (0~P-1)
	vector<int> anspath[MAXR];	//result_path record (eid)
	
	bool id_cmp(int i,int j) {
		if(dist[tasks[i]][taskt[i]]==dist[tasks[j]][taskt[j]]){
			return repath[i]>repath[j];
		}
...
namespace prep {
	...
	vector<PII> prepath[MAXR][candi_cnt];
	
	//length <= min_dist+tolerance treated as shortest path
	int tolerance=0;
	bitset<N> vis;
	int s,t,tid,cid,path[N+10];
...
namespace path {
	void getanspath01(int tid,int path_choose,int way_choose,int op) {
		way[op]=way_choose;
		...
		for(PII &pr:prepath[tid][path_choose]) {
			...
			if(!evis[eid].test(way_choose)) {
				anspath[op].push_back(eid);
				evis[eid].set(way_choose);
				break;
			}
		}
	}
	void getanspath(int tid,bitset<MAXP> &vis,int path_choose,int op) {
		//find first available way
		for(int i=0; i<P; ++i)
		if(!vis.test(i))
			return getanspath01(tid,path_choose,i,op);
		//assert(false);
		
	}
	void getanspath2(int tid,bitset<MAXP> &vis,int path_choose,int op,int ww) {
		//find first available way
		if(!vis.test(ww))
			return getanspath01(tid,path_choose,ww,op);
		//assert(false);
		
	}
	bitset<N> vis;
	int preeid[N];
	bool bfs01(int tid,int way_choose,int op) {
		...
		while(!q.empty()) {
			...
			if(cur==t) {
				...
				reverse(anspath[op].begin(),anspath[op].end());
				return true;
			}
			vector<int> tos;
			for(edge &e:edges[cur])
	...
	bool bfs2(int tid,int way_choose,int op){
		...
		while(!q.empty()) {
			...
			vector<int> tos;
			for(edge &e:edges[cur])
			if(!vis.test(e.to) && !evis[e.id].test(way_choose)&&!visM[e.id]) {
				vis.set(e.to);
				tos.push_back(e.to);
				preeid[e.to]=e.id;
			}
	...
	void change(int tid){
		for(int i=0;i<anspath[tid].size();i++){
			visM[anspath[tid][i]]=1;
		}
	}
	void rechange(int tid){
		for(int i=0;i<anspath[tid].size();i++){
			visM[anspath[tid][i]]=0;
		}
	}
}
...
void solve() {
	...
	for(int w=0; w<P; ++w) {
		...
		if(!anspath[rehw[i]].size()) {
			...
			if(!vis){
				getanspath01(i,path_choose,w,rehw[i]);
				//getway[i]=path_choose;
			}
		}
		//try bfs
		for(int idx=0,i=sq[idx],cnt=0.3*T; idx<cnt; ++idx,i=sq[idx])
			if(!anspath[rehw[i]].size())
				bfs01(i,w,rehw[i]);
	}
	//bfs else brute force the prepath
	for(int idx=0,i=sq[idx]; idx<T; ++idx,i=sq[idx])
	if(!anspath[rehw[i]].size() && !bfs(i,rehw[i])) {
		bitset<MAXP> vis,tmp;
		...
		getanspath(i,vis,path_choose,rehw[i]);
	}
	//xin ye wu
	// for(int i=0;i<6;i++){
	// 	if(!anspath[i].empty())
	// 		cout<<anspath[i].size()<<"??"<<anspath[i][0]<<"??"<<i<<endl;
	// }
	for(int idx=0,i=sq[idx]; idx<T; ++idx,i=sq[idx]){
		
		if(repath[i]==2){
			//cout<<anspath[i].size()<<"??"<<i<<endl;
			int Way=way[rehw[i]];
			//cout<<way[rehw[i]]<<"??"<<endl;
			//cout<<i<<":::"<<Way<<"??"<<anspath[rehw[i]-1][0]<<endl;
			change(rehw[i]);
			for(int j=rehw[i]+1;j<rehw[i]+repath[i];j++){
				if(bfs2(i,Way,j)){
					...
					for(PII &pr:prepath[i][path_choose]) {
						...
						//add edge
						//tmp.set(Way);
						if(!tmp.test(Way))vis|=tmp;
						else adde(m+execnt++,pr.first,pr.second,mindis[pr]);
					}
					//cout<<Way<<"??"<<endl;
					getanspath2(i,vis,path_choose,j,Way);
				}
			}
			...
			for(int j=rehw[i]+1;j<rehw[i]+repath[i];j++){
				...
				if(!anspath[j].size()){
					...
					for(PII &pr:prepath[i][path_choose]) {
						...
						// if((vis|tmp).count()!=(unsigned)P)vis|=tmp;
						// else 
						adde(m+execnt++,pr.first,pr.second,mindis[pr]);
					}
					// assert(vis.count()!=(unsigned)P);
					getanspath(i,vis,path_choose,j);
					//assert(anspath[j].size());
				}
				change(j);
			}
			for(int j=rehw[i];j<rehw[i]+repath[i];j++){
				rechange(j);
			}
		}
	}
...
int main() {
	...
	for(int i=m; i<m+execnt; ++i)
		printf("%d %d\n",es[i].from,es[i].to);
	for(int i=0; i<R; ++i) {
		int d=0,expcnt=0,cur=tasks[hw[i]];
