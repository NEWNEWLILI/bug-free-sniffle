...
namespace info {
	...
	int n,m,T,P,D,R;
	int tasks[MAXT],taskt[MAXT],repath[MAXT];
	int visT[MAXT];
	map<int,int> hw,rehw;
	//edges
	struct edge {
...
namespace mind {
	...
	struct dj_cmp {
		...
		bool operator () (PII &p1,PII &p2) {
			//dist1+1 > dist2+1
			// if(dist[curi][p1.first]==dist[curi][p2.first]){
			// 	return repath[]
			// }
...
namespace path {
	...
	void getanspath(int tid,bitset<MAXP> &vis,int path_choose) {
		...
		for(int i=0; i<P; ++i)
		if(!vis.test(i))
			return getanspath01(tid,path_choose,i);
	}
...
void solve() {
	...
	for(int w=0; w<P; ++w) {
		...
		if(!anspath[i].size()) {
			...
			if(!vis)getanspath01(i,path_choose,w);
		}
		//try bfs
