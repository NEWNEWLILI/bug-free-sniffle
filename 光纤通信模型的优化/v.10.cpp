...
void solve() {
	...
	for(int w=0; w<P; ++w) {
		//try prepath
        // for(int idx=0,i=sq[idx],cnt=T; idx<cnt; ++idx,i=sq[idx]){
        //     //cout<<dist[tasks[i]][taskt[i]]<<"??"<<endl;
        //     if(!anspath[i].size()&&dist[tasks[i]][taskt[i]]<=1) {
		// 	    bool vis;
		// 	    int path_choose=0;
		...
		for(int idx=0,i=sq[idx],cnt=0.888*T; idx<cnt; ++idx,i=sq[idx])
			if(!anspath[i].size()&&((query[{tasks[i],taskt[i]}]%P)>=(P/5)))
				bfs01(i,w);
	}
	int cntt=0;
	...
	if(!anspath[i].size() && !bfs(i)) {
		//cntt+=dist[tasks[i]][taskt[i]];
		int path_choose=0;
		for(int j=0;j<prep::candi_cnt;j++){
			int num=0,check=9999999;
			bitset<MAXP> vis,tmp;
			for(PII &pr:prepath[i][path_choose]) {
				tmp.set();
				vector<int> &eids=eds[pr];
