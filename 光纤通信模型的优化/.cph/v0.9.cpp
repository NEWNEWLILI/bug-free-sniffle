...
namespace {
	...
	void getpath(int tid) {
		...
		while(cur!=t) {
			mind--;
            int op=-999999999,pos=-1,idd=-1;
			vector<int> candidate;
			for(edge &e:edges[cur])
			if(dist[e.to][t]==mind){
                //cout<<idd<<"??"<<visT[e.to]<<endl;
                candidate.push_back(e.id);
                //cout<<visT[e.to]<<"??"<<candidate.size()<<endl;
                if(visT[e.to]>op){
                    pos=candidate.size();
                    op=visT[e.to];
                    idd=e.to;
...
int main() {
    ...
    for(int idx=0,i=sq[0]; idx<T; ++idx,i=sq[idx])
		if(!anspath[i].size()){
            for(int j=0;j<10;j++){
                int len=prepath[i][j].size();
                for(int p=0;p<len;p++){
                    if(p==0){
                        visT[prepath[i][j][p].first]++;
                        visT[prepath[i][j][p].second]++;
                    }
                    else{
                        visT[prepath[i][j][p].second]++;
                    }
